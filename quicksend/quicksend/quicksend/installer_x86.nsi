...
OutFile "dist\QuickSend_Setup_x86_v6.0.exe"
...
Section "MainSection" SEC01
  ...
  File /r "dist\QuickSend\*.*"
  
  CreateDirectory "$SMPROGRAMS\QuickSend"
  CreateShortCut "$SMPROGRAMS\QuickSend\QuickSend.lnk" "$INSTDIR\QuickSend.exe" "" "$INSTDIR\QuickSend.exe" 0
  CreateShortCut "$DESKTOP\QuickSend.lnk" "$INSTDIR\QuickSend.exe" "" "$INSTDIR\QuickSend.exe" 0
SectionEnd
...
Section -Post
  ...
  IfFileExists "$INSTDIR\vc_redist.x86.exe" 0 +3
